...
class AmbienceAIMCPServer {
  ...
  private setupHandlers() {
    ...
    this.server.setRequestHandler(CallToolRequestSchema, async (request, extra) => {
      ...
      try {
        const result = await tools.executeTool(request.params.name, request.params.arguments || {});
        return result;
      } catch (error) {
        return {
          content: [{
  ...
  async start() {
    ...
    console.error('Available tools: get_credits, generate_image, generate_video, generate_audio, get_library, get_creation_status');
  }
}
