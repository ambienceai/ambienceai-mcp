...
describe("CreationSchema", () => {
  ...
  it("validates status enum", () => {
    expect(() =>
      CreationSchema.parse({ ...validCreation, status: "invalid" }),
    ).toThrow();
  });

  it("validates type enum", () => {
    expect(() =>
      CreationSchema.parse({ ...validCreation, type: "invalid" }),
    ).toThrow();
  });
  ...
  it("allows all valid statuses", () => {
    ["pending", "processing", "completed", "failed"].forEach((status) => {
      expect(() =>
        CreationSchema.parse({ ...validCreation, status }),
      ).not.toThrow();
    });
  });

  it("allows all valid types", () => {
    ["image", "video", "speech", "music", "upscale", "transcription"].forEach(
      (type) => {
        expect(() =>
          CreationSchema.parse({ ...validCreation, type }),
        ).not.toThrow();
      },
    );
  });
...
describe("GenerateImageRequestSchema", () => {
  it("parses minimal valid request", () => {
    ...
    // No client-side model default — the server resolves omitted models
  ...
  it("passes new models like nano_banana_2 through without a schema change", () => {
    const result = GenerateImageRequestSchema.parse({
      prompt: "test",
      model: "nano_banana_2",
    });
    expect(result.model).toBe("nano_banana_2");
  });
  ...
  it("validates aspectRatio enum", () => {
    expect(() =>
      GenerateImageRequestSchema.parse({ prompt: "test", aspectRatio: "2:1" }),
    ).toThrow();
  });
  ...
  it("allows all valid aspect ratios", () => {
    ["16:9", "9:16", "1:1", "4:3", "3:4"].forEach((aspectRatio) => {
      expect(() =>
        GenerateImageRequestSchema.parse({ prompt: "test", aspectRatio }),
      ).not.toThrow();
    });
  });
  ...
  it("accepts file paths for guideImageUrl", () => {
    const result = GenerateImageRequestSchema.parse({
      prompt: "test",
      guideImageUrl: "/path/to/guide.png",
    });
...
describe("GenerateVideoRequestSchema", () => {
  ...
  it("rejects duration less than 1", () => {
    expect(() =>
      GenerateVideoRequestSchema.parse({ prompt: "test", duration: 0 }),
    ).toThrow();
  });

  it("rejects duration greater than 30", () => {
    expect(() =>
      GenerateVideoRequestSchema.parse({ prompt: "test", duration: 31 }),
    ).toThrow();
  });
...
describe("GenerateSpeechRequestSchema", () => {
  ...
  it("allows all valid languages", () => {
    ...
    languages.forEach((language) => {
      expect(() =>
        GenerateSpeechRequestSchema.parse({ text: "test", language }),
      ).not.toThrow();
    });
  });
  ...
  it("rejects speed less than 0.25", () => {
    expect(() =>
      GenerateSpeechRequestSchema.parse({ text: "test", speed: 0.1 }),
    ).toThrow();
  });

  it("rejects speed greater than 4.0", () => {
    expect(() =>
      GenerateSpeechRequestSchema.parse({ text: "test", speed: 5 }),
    ).toThrow();
  });
...
describe("GenerateAudioRequestSchema (legacy)", () => {
  it("parses speech type", () => {
    const result = GenerateAudioRequestSchema.parse({
      prompt: "hello",
      type: "speech",
    });
  ...
  it("requires type field", () => {
    expect(() =>
      GenerateAudioRequestSchema.parse({ prompt: "test" }),
    ).toThrow();
  });
});
...
describe("UpscaleImageRequestSchema", () => {
  ...
  it("rejects upscaleFactor less than 1", () => {
    expect(() =>
      UpscaleImageRequestSchema.parse({
...
describe("TranscribeAudioRequestSchema", () => {
  ...
  it("rejects invalid URL", () => {
    expect(() =>
      TranscribeAudioRequestSchema.parse({ audioUrl: "not-a-url" }),
    ).toThrow();
  });
});
