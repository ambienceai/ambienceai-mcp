...
describe('GENERATION_TYPES', () => {
  it('has all expected generation types', () => {
    ...
    expect(GENERATION_TYPES.IMAGE_UPSCALE).toBe('image_upscale');
    ...
    expect(GENERATION_TYPES.AUDIO_TRANSCRIPTION).toBe('audio_transcription');
  });
});
...
describe('ESTIMATED_DURATIONS', () => {
  it('has duration for each generation type', () => {
    ...
    expect(ESTIMATED_DURATIONS[GENERATION_TYPES.AUDIO_TRANSCRIPTION]).toBe(45);
  });
});
